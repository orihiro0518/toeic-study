...
const meaningOptions=c=>[...new Set(questions.filter(q=>q.c===c&&q.meaning).map(q=>senseText(q)))];
function choiceMeanings(q){const correct=senseText(q),rest=shuffled(meaningOptions(q.c).filter(x=>x!==correct)).slice(0,3);return shuffled([correct,...rest])}
...
function senseHtml(q){if(q.c!=='vocab'||!q.senses?.length)return'';return '<div class="senseBox"><strong>品詞別の意味</strong>'+q.senses.map(x=>'<div class="senseLine"><span class="sensePos">'+escapeHtml(x[0])+'</span>'+escapeHtml(x[1])+'</div>').join('')+'</div>'}
...
window.renderQuestion=function(){
  ...
  if(q.c==='vocab'||q.c==='phrase'){
    qt.textContent=shown(q);q._meanOpts=choiceMeanings(q);q._correct=q._meanOpts.indexOf(senseText(q));
...
window.answer=function(i,b){
  ...
  if(q.c==='vocab'||q.c==='phrase'){
    const sentence=q.q.replace('_____',term(q)),meaning=senseText(q),important=q.important?'<div style="margin:8px 0;color:#ffe08a;font-weight:900">★ TOEICで重要度が高い語</div>':'';
    e.innerHTML='<b>'+(i===correct?'正解！':'正解：'+String.fromCharCode(65+correct)+'. '+escapeHtml(meaning))+'</b><br><strong>'+escapeHtml(shown(q))+' = '+escapeHtml(meaning)+'</strong>'+important+senseHtml(q)+'<br>📝 例文：'+escapeHtml(sentence)+'<br><br>📘 '+escapeHtml(q.e);
  }else{
...
window.renderList=function(){
  ...
  questions.forEach((q,i)=>{
    if(cat!=='all'&&q.c!==cat)return;const text=q.c==='grammar'?q.q:shown(q),ans=q.c==='grammar'?String.fromCharCode(65+q.a)+'. '+q.o[q.a]:senseText(q),category=q.c==='grammar'?'文法・'+(q.gc||'総合'):labels[q.c];
    ...
    const mark=q.important?'<span class="toeic-list-star">★重要</span> ':'';const answerHtml=q.c==='vocab'&&q.senses?.length?q.senses.map(x=>'<span class="sensePos">'+escapeHtml(x[0])+'</span>'+escapeHtml(x[1])).join('<br>'):escapeHtml(ans||'');
...
const vc=[...document.querySelectorAll('button.card')].find(b=>b.querySelector('h3')?.textContent.includes('単語問題'));if(vc){const p=vc.querySelector('p');if(p)p.textContent='全300問。複数の意味は品詞別に表示。★金色はTOEIC重要語。'}
const ver=document.querySelector('.ver');if(ver)ver.textContent='ver 1.5.0';
})();